...
folder = "E0_stuff"
sup_estimator = pickle.load(open(folder + "/sup_estimator_pre_lin_5.pkl", 'rb'))
tot_estimator = pickle.load(open(folder + "/tot_estimator_pre_lin_5.pkl", 'rb'))
...
for s in np.arange(-80, 81):
    ...
    for t in np.arange(200, 401):
        ...
        value = sup_estimator.best_estimator_.predict(np.asarray([[sup, tot, abs(sup), sup if sup > 0 else 0, -sup if sup <0 else 0]]))[0]
        value2 = tot_estimator.best_estimator_.predict(np.asarray([[sup, tot, abs(sup), sup if sup > 0 else 0, -sup if sup <0 else 0]]))[0]
...
df = pd.read_csv("E0_underlying_1817.csv")
...
def run_league(args, should_print=False, write_csv=False):
          ...
          #          print ("ha", model.coef_[-1])
               ...
               #         fake_row_2[-1] = -1
                        ...
                        predicted_value_2 = model_pred(model2, [fake_row_2])[0] + total_adjust[home_ind] + total_adjust[
                            away_ind]
                        actual_sup_prd = sup_estimator.best_estimator_.predict(np.asarray([[predicted_value,
                                                                                            predicted_value_2,
                                                                                            abs(predicted_value),
                                                                                            predicted_value if predicted_value > 0 else 0,
                                                                                            -predicted_value if predicted_value < 0 else 0]]))[0]
                        actual_tot_prd = \
                        tot_estimator.best_estimator_.predict(np.asarray([[predicted_value,
                                                                                            predicted_value_2,
                                                                                            abs(predicted_value),
                                                                                            predicted_value if predicted_value > 0 else 0,
                                                                                            -predicted_value if predicted_value < 0 else 0]]))[0]
                        ...
                        if date_as_dt2 < datetime(2018, 5, 1):
                            #hc bet
                            if ((actual_sup_prd* actual_tot_prd) - game2[-4] * game2[-3]) > bm.hc_th:
                                bm.hc_bets += 1
                                if game2[4] - game2[5] + game2[6] >= 0.5:
                                    bm.hc_pl += 100 * (game2[7] -1)
                                elif game2[4] - game2[5] + game2[6] == 0.25:
                                    bm.hc_pl += 100 * (game[7] -1)/2
                                elif game2[4] - game2[5] + game2[6] == 0:
                                    pass
                                elif game2[4] - game2[5] + game2[6] == -0.25:
                                    bm.hc_pl -= 100/2
                                else:
                                    bm.hc_pl -= 100
                            elif ((actual_sup_prd* actual_tot_prd) - game2[-4] * game2[-3]) < -bm.hc_th:
                                bm.hc_bets += 1
                                if game2[5] - game2[4] - game2[6] >= 0.5:
                                    bm.hc_pl += 100 * (game2[8] - 1)
                                elif game2[5] - game2[4] - game2[6] == 0.25:
                                    bm.hc_pl += 100 * (game2[8] - 1) / 2
                                elif game2[5] - game2[4] - game2[6] == 0:
                                    pass
                                elif game2[5] - game2[4] - game2[6] == -0.25:
                                    bm.hc_pl -= 100 / 2
                                else:
                            ...
                            # ou bet
                            if ((actual_tot_prd) - game2[-3]) > bm.ou_th:
                                bm.ou_bets += 1
                                if game[4] + game2[5] - 2.5 >= 0.5:
                                    bm.ou_pl += 100 * (game2[9] - 1)

                                else:
                                    bm.ou_pl -= 100
                            elif (( actual_tot_prd) - game2[-3]) < -bm.hc_th:
                                bm.ou_bets += 1
                                if game2[5] + game2[4] - 2.5 <= -0.5:
                                    bm.ou_pl += 100 * (game2[10] - 1)

                                else:
                                    bm.ou_pl -= 100

                      #  get_team_ranks(team_list, model2.coef_)
                        ...
                        differences2.append([[predicted_value_2], game2[-1]])
    ...
    differences2 = np.array(differences2)
    #   print(differences)

    differences2 = np.abs(differences2[:, 0] - differences2[:, 1])
    #   print (args)
    print("Hc")
    print(np.mean(differences), np.max(differences))
    print("tot")
    print(np.mean(differences2), np.max(differences2))

    print(bm.hc_pl, bm.hc_bets, bm.hc_pl / (100 * bm.hc_bets))
    print(bm.ou_pl, bm.ou_bets, bm.ou_pl / (100 * bm.ou_bets))
