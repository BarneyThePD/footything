...
for s in np.arange(-80, 81):
    ...
    for t in np.arange(200, 401):
        ...
        value = sup_estimator.best_estimator_.predict(np.array([[sup, tot]]))[0]
        value2 = tot_estimator.best_estimator_.predict(np.array([[sup, tot]]))[0]
...
sup_list = np.array(sup_list)
tot_list = np.array(tot_list)
sup_value_list = np.array(sup_value_list)
tot_value_list = np.array(tot_value_list)
        ...
        if date_as_dt < datetime(2022, 3, 2):
          ...
          #          print ("ha", model.coef_[-1])
               ...
               #         fake_row_2[-1] = -1
                        ...
                        predicted_value_2 = model_pred(model2, [fake_row_2])[0] + total_adjust[home_ind] + total_adjust[away_ind]
                        actual_sup_prd = sup_estimator.best_estimator_.predict(np.array([[predicted_value, predicted_value_2]]))[0]
                        actual_tot_prd = \
                        tot_estimator.best_estimator_.predict(np.array([[predicted_value, predicted_value_2]]))[0]
                        ...
                        if game2[4] + game2[5] == 0:
                            ...
                            sup_adjust[home_ind] = ((game2[4] - game2[5]) / (game2[4] +  game2[5]) - game2[-4]) * game_carry
                            sup_adjust[away_ind] = ((-game2[4] + game2[5]) / (game2[4] + game2[5]) + game2[-4]) * game_carry
...
#optim = minimize(run_league, np.array([1, 40]), method="BFGS")
